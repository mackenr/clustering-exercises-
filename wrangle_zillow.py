...
def prep_zillow_2017(k=1.25):
    ...
    # all=['bathroomcnt','bedroomcnt','calculatedfinishedsquarefeet','taxvaluedollarcnt','fips','yearbuilt','parcelid','lotsizesquarefeet','logerror']
    mvp=['bathroomcnt','bedroomcnt','calculatedfinishedsquarefeet','taxvaluedollarcnt','fips','yearbuilt','lotsizesquarefeet','parcelid']
  
    todrop=set(mvp)-{'parcelid'}
    ...
    df=pd.merge(df_6037, df_6059, how='outer',on=mvp)
    df=pd.merge(df, df_6111, how='outer',on=mvp)
    ...
    df.drop(columns=['parcelid','fullbathcnt','calculatedbathnbr'],inplace=True)
